...
const TriggerNode = ({ data, selected, id }) => {
    const { label, triggerType, icon, color, onDelete } = data;
        ...
        >
            ...
            <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1">
                    <div className="text-xs text-text-muted uppercase font-semibold">Trigger</div>
                    <div className="text-sm font-bold text-text-main">{label}</div>
                </div>
            </div>

            {/* Trigger Type Badge */}
            <div className="bg-green-500/20 text-green-400 text-xs px-2 py-1 rounded inline-block">
                {triggerType}
            </div>
